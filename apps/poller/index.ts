...
interface SwapConfig {
  ...
  usdcEthereumAddress: string; // PYUSD address for cross-chain swaps
  srcChainId: number;
  dstChainId: number;
  pollInterval: number;
}
...
class CrossChainSwapService {
  ...
  private async getQuote(amount: bigint, makerAddress: string): Promise<Quote> {
    console.log("📊 Fetching quote for SOL → PYUSD cross-chain swap...");

    const srcToken = SolanaAddress.fromString(this.config.solTokenAddress);
    const dstToken = EvmAddress.fromString(this.config.usdcEthereumAddress); // Using PYUSD
  ...
  private async createAndSubmitOrder(
    ...
    console.log("📝 Creating SOL → PYUSD cross-chain order...");
    ...
    // DISCOVERY: Get both hash formats - use Solana hash for monitoring (tested working)
    ...
    // Use Solana-style hash for monitoring (confirmed working in tests)
    ...
    if (ethereumOrderHash.length === 66 && ethereumOrderHash.startsWith("0x")) {
      console.log("✅ PERFECT! Got proper 66-character Ethereum order hash");
      console.log(
        "🎯 But using 44-character Solana hash for monitoring (tested working)"
      );
    } else if (orderHash.length >= 43 && orderHash.length <= 44) {
      console.log("✅ Using 44-character Solana hash for monitoring APIs");
    } else {
  ...
  private async monitorAndSubmitSecrets(
    ...
    console.log("👀 Starting AGGRESSIVE monitoring for fills...");
    console.log(`🔑 Generated ${secrets.length} secrets for submission`);
    console.log("🚀 Using 3-second polling to beat refund timeout!");

    // Following 1inch docs pattern but with AGGRESSIVE secret submission
    const POLL_INTERVAL = 3000; // 3 seconds - MORE AGGRESSIVE
    const MAX_MONITORING_TIME = 10 * 60 * 1000; // 10 minutes max
    const startTime = Date.now();

    // Reduce initial wait time to be more responsive
    await setTimeout(2000); // Only 2 seconds instead of full poll interval
    ...
    while (true) {
      try {
        ...
        console.log(
          `📊 Order status: ${order.status} (${minutesElapsed} min elapsed)`
        );
      ...
      try {
        // Check for ready secrets VERY AGGRESSIVELY (from 1inch docs)
        ...
        // Submit secrets IMMEDIATELY and AGGRESSIVELY (from 1inch docs)
        for (const idx of idxes) {
          if (!alreadyShared.has(idx)) {
            try {
              const secret = secrets[idx];
              if (secret) {
                console.log(`🔑 URGENT: Submitting secret for index ${idx}...`);
                await this.sdk.submitSecret(orderHash, secret);
                alreadyShared.add(idx);
                console.log("✅ FAST SUBMITTED secret for index:", idx);
                console.log("🎯 Secret submission should prevent refund!");
              } else {
        ...
        console.log(
          `🔄 Aggressive polling continues (${POLL_INTERVAL}ms interval)...`
        );
      } catch (err) {
  ...
  public async performCrossChainSwap(
    ...
    try {
      console.log(`🔄 Starting SOL → PYUSD cross-chain swap for ${user.email}`);
      ...
      // CRITICAL: Minimum swap amount validation for economic viability
      const minimumSwapSOL = 0.02; // 0.02 SOL minimum for resolver profitability
      if (solToWrap < minimumSwapSOL) {
        throw new Error(
          `Swap amount too small: ${solToWrap} SOL. Minimum viable amount: ${minimumSwapSOL} SOL. ` +
            `Small orders get refunded because they're unprofitable for resolvers due to Ethereum gas costs.`
        );
      }
...
class SolanaBalancePoller {
  ...
  constructor() {
    ...
    this.minSolBalance = parseFloat(process.env.MIN_SOL_BALANCE || "0.025"); // Increased for viable swaps
    ...
    const swapConfig: SwapConfig = {
      ...
      usdcEthereumAddress:
        ...
        "0x6c3ea9036406852006290770BEdFcAbA0e23A0e8", // Official PYUSD mainnet address
    ...
    console.log("🎯 PRODUCTION MODE: SOL → PYUSD");
    console.log("� Cross-chain swap: Solana SOL to Ethereum PYUSD");
